"""
Enhanced Safety Filter v2 for Open WebUI
Filters user inputs and model outputs for harmful content using a safety model.
The classification logic is defined in the safety model's system prompt.
...
class Filter:
    ...
    async def check_safety(
        ...
        try:
            ...
            # Build prompt (content only - model's system prompt defines classification logic)
            ...
            # The safety model's system prompt contains all classification instructions
            try:
            ...
            # Send message with minimal structure for vLLM compatibility
            # Many models (Llama Guard, ShieldGemma) require strict user/assistant alternation
            # and don't support system messages - only send user message
            payload = {
    ...
    def build_safety_prompt(self, content: str) -> str:
        """
        Returns the content to be checked by the safety model.
        
        The safety model's system prompt (configured in OpenWebUI) contains all
        classification instructions. This method simply returns the content to analyze.
        
        Example system prompt should define:
        - What constitutes unsafe content
        - Response format (e.g., "safe" or "unsafe [category]")
