...
class Filter:
    ...
    def _mask(self, value: Any) -> Any:
        # Redact common secret keys and PII patterns
        try:
            import re
        except Exception:
            re = None  # Fallback if re import fails unexpectedly
        ...
        def mask_str(s: str) -> str:
            ...
            if re:
                ...
                # Phone-like sequences (very rough)
                out = re.sub(r"\+?\d[\d\-\.\s()]{6,}\d", "[REDACTED_PHONE]", out)
                # Bearer/JWT/API tokens (long base64/hex-like)
                out = re.sub(r"([A-Za-z0-9-_]{20,})", "[REDACTED_TOKEN]", out)
            return out
        ...
        if isinstance(value, dict):
            ...
            for k, v in value.items():
                kl = str(k).lower()
                if kl in {"authorization", "api_key", "apikey", "token", "secret", "password"}:
                    redacted[k] = "[REDACTED]"
                else:
                    redacted[k] = self._mask(v)
            return redacted
    ...
    async def inlet(
        ...
        if last_message.get("role") == "user":
            ...
            self._dbg_step(f"Injection check result: is_safe={is_injection_safe} reason={self._mask(injection_reason)}")
            ...
            if not is_injection_safe and self.valves.block_on_unsafe:
                ...
                self._dbg_step(f"Blocking input content: Prompt Injection detected - {self._mask(injection_reason)}")
                if __event_emitter__:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if resp.status == 200:
                        ...
                        if file_id:
                            # Mask file_id display in step logs
    ...
    async def check_safety(
        ...
        try:
            ...
            if (
                ...
                self._dbg_step("Augmenting with compliance KB")  # Log KB augmentation start
...
{sanitized_content}"""
                self._dbg_full("Summary prompt:", self._truncate(summary_prompt, 200))  # heavy
                ...
                self._dbg_full("Summary payload:", summary_payload)  # heavy
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                        ...
                        self._dbg_step(f"Summary response status: {resp.status}")  # Log status
                        if resp.status == 200:
                            summary_result = await resp.json()
                            self._dbg_full("Summary result:", summary_result)  # heavy
                            query_summary = (
                            ...
                            self._dbg_step(f"Query summary: '{self._truncate(query_summary, 200)}'")  # Log summary
                            if not query_summary:  # Strict: If empty, block
                                self._dbg_step("Empty summary - blocking as potential injection")  # Log reason
                                return False, "Potential injection detected"
                        else:
                            resp_text = await resp.text()
                            self._dbg_full(f"Summary failed: {self._truncate(resp_text, 200)}")  # failure details
                ...
                for kb_name in kb_names:
                    try:
                        ...
                        self._dbg_step(f"Querying KB '{kb_name}' with: '{self._truncate(policy_query, 200)}'")  # Log query
                ...
                self._dbg_step(f"Policy snippets: '{self._truncate(policy_snippets, 200)}...'")  # truncated
                ...
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        if isinstance(data, list):
                            ...
                            self._dbg_full(f"Unexpected models response format: {type(data)}")
                            return []
                        
                        self._dbg_full(f"Fetched {len(models)} models from API")
                        self.available_models = models
                        return models
                    else:
                        resp_text = await response.text()
                        self._dbg_step(f"Failed to fetch models: {self._mask(resp_text)}")
                        return []
        except Exception as e:
    ...
    def _resolve_model_id(self, model_display_name: str) -> str:
        ...
        self._dbg_step(f"Model '{model_display_name}' not found in {len(self.available_models)} models - returning original")
    ...
    async def _detect_prompt_injection_semantic(self, content: str) -> tuple[bool, str]:
        ...
        Returns:
            tuple: (is_safe: bool, reason: str)
        """
        self._dbg_step(f"Starting semantic injection detection for content: '{self._mask(content)}'")
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                self._dbg_full(f"Querying injection detection model with payload: {self._mask(str(payload))}")
                ...
                async with session.post(
                    ...
                    if response.status != 200:
                        resp_text = await response.text()
                        self._dbg_step(f"Injection detection error: {self._mask(resp_text)}")
                        return False, f"Model '{self.valves.injection_detection_model}' error - blocking as unsafe"
                    
                    result = await response.json()
                    self._dbg_full(f"Injection detection full result: {self._mask(str(result))}")
                    response_text = (
                        result.get("choices", [{}])[0]
                        .get("message", {})
                        .get("content", "")
                    ...
                    self._dbg_step(f"Injection detection response: {self._mask(response_text)}")
                    ...
                    self._dbg_step(f"Semantic injection result: is_safe={is_safe}, reason={self._mask(reason)}")
...
Response:"""
        # Removed duplicate prompt logging; logged in check_safety() under Full Debug
        return prompt
